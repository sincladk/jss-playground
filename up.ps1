...
dotnet sitecore login --cm https://cm.jss_playground.localhost/ --auth https://id.jss_playground.localhost/ --allow-write true
if ($LASTEXITCODE -ne 0) {
...
Write-Host "accessTokenValue: $token"
Invoke-RestMethod "https://cm.jss_playground.localhost/sitecore/admin/PopulateManagedSchema.aspx?indexes=all" -Headers @{Authorization = "Bearer $token"} -UseBasicParsing | Out-Null
...
Start-Process https://cm.jss_playground.locadown    lhost/sitecore/
Start-Process https://www.jss_playground.localhost/
